...
(defmacro deftemplate [sym args & forms]
  (let [{:keys [compiled location]
         :or   {compiled false}} (meta sym)]
    (assert location  ;; make sure we got everything we need.
            (format "Please specify :location for %s/%s" *ns* sym))
    `(do
       ~(if compiled
          (load-local-dom location)
          `(enfocus.core/load-remote-dom ~location))
       (enfocus.macros/create-dom-action
        ~sym
        #(enfocus.core/get-cached-dom ~location)
        true ~args ~@forms))))

(defmacro defsnippet [sym args & forms]
  (let [{:keys [compiled location selector]
         :or {compiled false}} (meta sym)]
    (assert (and location selector)
            (format "Please specify :location and :selector for %s/%s"
                    (keyword key) *ns* sym))
    `(do
       ~(if compiled
          (load-local-dom location)
       ...
       (enfocus.macros/create-dom-action
        ~sym
        #(enfocus.core/get-cached-snippet ~location ~selector)
        true ~args ~@forms))))
